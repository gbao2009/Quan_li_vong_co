...
function handleData(raw){
  ...
  if(row.valid){
    ...
    s.t.push(t);
        s.temperature.push(row.temperature);
     s.bpm.push(row.bpm);
...
function renderDashboard(){
  ...
  document.getElementById(
   ...
   document.getElementById(
     ...
     `${Number(r.temperature || 0).toFixed(2)}<span class="u">°C</span>`;
...
function renderFullTelemetry(
  ...
  const items = [
    ...
    ["🌡️ Nhiệt độ", Number(r.temperature || 0).toFixed(2) + " °C"],
     ["RED", r.red],
    ["IR", r.ir],
...
function renderChartStatuses(){
  ...
  ["statBpm","statSpo2","statTemperature","statAcc","statGyro"]
    .forEach(elId => {
  ...
  ["statDetailBpm","statDetailSpo2","statDetailTemperature"]
    .forEach(elId => {
  ...
  temperatureChart =
    new Chart(
      ...
      ),
      {
        type:"line",
        data:{datasets:[
          makeDataset("Nhiệt độ", css("--warn"), "°C")
        ]},
        options:chartOptions()
      }
    );
...
function updateMainCharts(
  ...
  spo2Chart.update("none");
   ...
   temperatureChart.data.datasets[0].data =
     toXY(s.t, s.temperature);

   slideChartWindow(temperatureChart);
            ...
            <div class="dc-stats">
              ...
              <div class="dc-stat">
               ...
               <div class="dc-stat">
                 ...
                 <b>${Number(r.temperature || 0).toFixed(2)} °C</b>
               </div>
...
function renderDetail(){
  ...
  document.getElementById(
   ...
   document.getElementById(
     ...
     Number(r.temperature || 0).toFixed(2) + "°C";
  ...
  detailSpo2Chart.update("none");
   ...
   detailTemperatureChart.data.datasets[0].data =
     toXY(s.t, s.temperature);

   slideChartWindow(detailTemperatureChart);
   detailTemperatureChart.update("none");
      ...
      .map(
        r => `
          <tr>
            ...
            <td>${r.avg}</td>
            <td>${r.spo2}</td>
            <td>${Number(r.temperature || 0).toFixed(2)}</td>
             <td>${r.ir}</td>
            <td>${r.red}</td>
...
function renderHistory(){
  ...
  document.getElementById(
    ...
    rows
      ...
      `<tr><td colspan="15">Chưa có dữ liệu</td></tr>`;
}
...
function exportCSV(){
  ...
  const headers = [
    "Time","Device","BPM","AVG","SpO2","TemperatureC","IR","RED",
